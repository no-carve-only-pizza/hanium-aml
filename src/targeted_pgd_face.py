...
PGD is an iterative targeted attack. It stays inside an epsilon-ball around the
original image, so it is a natural next baseline after targeted FGSM.
"""
...
def main() -> None:
    ...
    for image, true_label in tqdm(samples, desc="targeted face PGD"):
        ...
        for _ in range(args.steps):
            ...
            logits = model(model_input(adv))
            loss = F.cross_entropy(logits, target)
            model.zero_grad(set_to_none=True)
            loss.backward()

            # Targeted PGD minimizes target-label loss.
